...
class SmartMoneyScalpingBacktest:
    def __init__(self, symbol: str, initial_balance: float = 1000.0, symbol_config: dict = None):
        ...
        # Configuración para MACD de 1 hora
        self.enable_1h_macd_filter = True
        self.macd_1h_fast = 12
        self.macd_1h_slow = 26
        self.macd_1h_signal = 9
        self.df_1h = None  # Almacenará los datos de 1h

    def fetch_historical_data(self, days=10) -> pd.DataFrame:
        """Descarga datos históricos para el timeframe principal (15m)."""
        return self._fetch_historical_data_for_timeframe(self.timeframe, days)
    
    def fetch_1h_historical_data(self, days=10) -> pd.DataFrame:
        """Descarga datos históricos para el timeframe de 1h."""
        return self._fetch_historical_data_for_timeframe('1h', days)
    
    def _fetch_historical_data_for_timeframe(self, timeframe: str, days: int) -> pd.DataFrame:
        """Método auxiliar para descargar datos históricos para cualquier timeframe."""
        try:
            ...
            while since < self.exchange.milliseconds():
                ...
                if len(all_ohlcv) % 20000 == 0: 
                    logger.info(f"Descargadas {len(all_ohlcv)} velas de {timeframe}...")
    ...
    def compute_macd(self, fast: int = None, slow: int = None, signal: int = None):
        """Calcula MACD clásico (EMA fast/slow + signal) sobre cierre para el timeframe principal."""
    ...
    def compute_macd_1h(self, fast: int = None, slow: int = None, signal: int = None):
        """Calcula MACD para el timeframe de 1h."""
        if self.df_1h is None or len(self.df_1h) == 0:
            logger.warning("No hay datos de 1h para calcular MACD.")
            return
            
        fast = fast or self.macd_1h_fast
        slow = slow or self.macd_1h_slow
        signal = signal or self.macd_1h_signal
        
        exp1 = self.df_1h['close'].ewm(span=fast, adjust=False).mean()
        exp2 = self.df_1h['close'].ewm(span=slow, adjust=False).mean()
        self.df_1h['macd'] = exp1 - exp2
        self.df_1h['macd_signal'] = self.df_1h['macd'].ewm(span=signal, adjust=False).mean()
        self.df_1h['macd_hist'] = self.df_1h['macd'] - self.df_1h['macd_signal']
        
        # Reindexar a 15m para facilitar la comparación
        self.df_1h_reindexed = self.df_1h.reindex(self.df.index, method='ffill')
        
        # Añadir columnas al dataframe principal para facilitar el acceso
        self.df['macd_1h'] = self.df_1h_reindexed['macd']
        self.df['macd_1h_signal'] = self.df_1h_reindexed['macd_signal']
        self.df['macd_1h_hist'] = self.df_1h_reindexed['macd_hist']
    ...
    def run_backtest(self):
        ...
        # Descargar datos del timeframe principal (15m)
        self.df = self.fetch_historical_data(days=30)
        if self.df is None or len(self.df) < 100:
            logger.error("No hay suficientes datos para realizar el backtest.")
            return
            
        # Si está habilitado el filtro de 1h, descargar datos de 1h
        if self.enable_1h_macd_filter:
            self.df_1h = self.fetch_1h_historical_data(days=30)
            if self.df_1h is not None and len(self.df_1h) > 0:
                self.compute_macd_1h()
            else:
                logger.warning("No se pudieron obtener datos de 1h. Desactivando filtro MACD 1h.")
                self.enable_1h_macd_filter = False
            ...
            if self.df['low'].iloc[i] <= fvg_high:
                ...
                if self.enable_macd_filter:
                    ...
                    if not (mh.iloc[i-1] > 0 and m.iloc[i-1] > ms.iloc[i-1] and mh.iloc[i-1] >= mh.iloc[i-2]):
                        return False
                # Verificar MACD 1h si está habilitado
                if self.enable_1h_macd_filter and i < len(self.df):
                    macd_1h_ok = (self.df['macd_1h_hist'].iloc[i] > 0)  # MACD 1h debe ser positivo
                    if not macd_1h_ok:
                        return False
    ...
    def check_short_setup(self, i):
        ...
        if not bearish_fvgs.empty:
            ...
            if self.df['high'].iloc[i] >= fvg_low:
                ...
                # Verificar MACD 1h si está habilitado
                if self.enable_1h_macd_filter and i < len(self.df):
                    macd_1h_ok = (self.df['macd_1h_hist'].iloc[i] < 0)  # MACD 1h debe ser negativo
                    if not macd_1h_ok:
                        return False
                pools = self.build_liquidity_pools(i, lookback=self.pool_lookback_bars, tol=self.equal_tol)
